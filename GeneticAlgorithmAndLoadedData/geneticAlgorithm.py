# this file runs all of the steps of the genetic algorithm and returns the results 
import time
import random
import operator
...
class paramSet:
	...
	# function that computes the scores for each of the different movies
    def computeScores(self):
		...
		# uses the sorted scores to create the list of nominees
        self.nominees = [n[0] for n in self.sortedScores[-20:]]
		# the nominee with the top score is the predicted winner
        self.winner = self.nominees[-1]

	# function that computes the fitness score
	...
	# creates the string of all of the variables and their values 
    def __str__(self):
...
# function that chooses the children from the parents and calculates the mutation
...
# chose the pairs to be used by the genetic algorithm 
def selectPairs(pop):
